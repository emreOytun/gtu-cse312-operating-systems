...
{
    // We have the pressed character coming here. We can print them if we want.
}
...
{
    // When the OS starts up, clean the keyboard buffer first by reading all characters in the buffer and ignoring them.
    while(commandport.Read() & 0x1)
        dataport.Read();
    
    // Activate keyboard interrupts by sending data(with using port.Write method) to the port.
...
uint32_t KeyboardDriver::HandleInterrupt(uint32_t esp)
{
    ...
    if(key < 0x80)
    {
        // To fill this, print the interrupt coming for each key, note down and map them here as below.
        switch(key)
        {
